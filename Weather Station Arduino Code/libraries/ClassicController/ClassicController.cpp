...
unsigned char ClassicController::begin(unsigned char which_controller)
{
    ...
    if (i2c_start_wait(WII_I2C_ADDR + I2C_WRITE, 10) == 1)
    {
...
void ClassicController::update()
{
    ...
    i2c_start_wait(WII_I2C_ADDR + I2C_WRITE, 0xFFFF);
    i2c_write(0x00);
    i2c_stop();

    delayMicroseconds(1000); // TODO is this needed?

    i2c_start_wait(WII_I2C_ADDR + I2C_READ, 0xFFFF);
    for (unsigned char i = 0; i < 5; i++)
    {
        data[i] = _decode_byte(i2c_readAck());
    }
    data[5] = _decode_byte(i2c_readNak());
    i2c_stop();
}
